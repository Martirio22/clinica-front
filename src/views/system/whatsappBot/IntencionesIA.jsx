...
const IntencionesIA = () => {
  ...
  const cambiarEstado = async (intent) => {
    const nuevoEstado = intent.isActive === false

    const confirmar = window.confirm(
      `¿Seguro que deseas ${nuevoEstado ? 'activar' : 'inactivar'} esta intención?`,
    )
    ...
    try {
      ...
      console.error(err)
      setError(err?.data?.message || err?.message || 'No se pudo cambiar el estado de la intención.')
    } finally {
      setLoading(false)
    }
  }

  const eliminarIntencion = async (intent) => {
    const confirmar = window.confirm(`¿Seguro que deseas eliminar o inactivar la intención ${intent.name}?`)
    ...
    try {
      ...
      setSuccess('Intención eliminada/inactivada correctamente.')
      await cargarIntenciones()
    } catch (err) {
      console.error(err)
  ...
  const verEventosIA = (intent) => {
    navigate(`/whatsapp-bot/eventos-ia?botIntentId=${intent.id}`)
  }
  ...
  const crearIntencionesBase = async () => {
    const intencionesBase = [
      {
        ...
        description: 'El paciente solicita información general de la clínica.',
      },
      {
        code: 'AGENDAR_CITA',
        name: 'Agendar cita',
        description: 'El paciente desea reservar una cita médica.',
      },
      {
        code: 'CONSULTAR_CITAS',
        name: 'Consultar citas',
        description: 'El paciente desea consultar sus citas pasadas o futuras.',
      },
      {
        code: 'CONSULTAR_RECETA',
        name: 'Consultar receta',
        description: 'El paciente solicita una receta médica emitida.',
      },
      {
        code: 'DERIVAR_ESPECIALISTA',
        name: 'Derivar especialista',
        description: 'La IA sugiere una especialidad según síntomas.',
      },
      {
        code: 'TRANSFERIR_HUMANO',
        name: 'Transferir a humano',
        description: 'El paciente solicita atención personalizada por asistente.',
      },
    ]
    ...
    try {
      ...
      setSuccess('Intenciones base creadas correctamente.')
      await cargarIntenciones()
    } catch (err) {
      console.error(err)
    ...
    <>
      <CCard>
        <CCardHeader className="d-flex justify-content-between align-items-center">
          ...
          <div>
            <CButton color="info" variant="outline" className="me-2" onClick={crearIntencionesBase}>
              Crear intenciones base
            </CButton>
        ...
        <CCardBody>
          ...
          <CRow className="mb-3">
            ...
            <CCol md={3} className="d-flex align-items-end">
              <CButton color="primary" variant="outline" className="w-100" onClick={cargarIntenciones}>
                Actualizar
              </CButton>
            </CCol>
          </CRow>
                ...
                {intencionesFiltradas.length === 0 ? (
                  <CTableRow>
                    <CTableDataCell colSpan={6} className="text-center">
                      No existen intenciones registradas.
                    </CTableDataCell>
                  </CTableRow>
                ) : (
                    ...
                    <CTableRow key={intent.id}>
                      ...
                      <CTableDataCell className="text-end">
                        ...
                        <CButton
                          ...
                          size="sm"
                          className="me-2 mb-1"
                          onClick={() => verEventosIA(intent)}
                        >
                          Ver eventos IA
                        </CButton>

                        <CButton
                          color={intent.isActive === false ? 'success' : 'danger'}
                          variant="outline"
                          size="sm"
                          className="me-2 mb-1"
                          onClick={() => cambiarEstado(intent)}
                        >
                          {intent.isActive === false ? 'Activar' : 'Inactivar'}
                        </CButton>
                        ...
                        <CButton
                          ...
                          size="sm"
                          className="mb-1"
                          onClick={() => eliminarIntencion(intent)}
                        >
                          Eliminar
                        </CButton>
                      </CTableDataCell>
      ...
      <CModal visible={visibleForm} onClose={cerrarForm} backdrop="static" size="lg">
        <CModalHeader>
          <CModalTitle>{editingIntent ? 'Editar intención' : 'Nueva intención'}</CModalTitle>
        </CModalHeader>
          ...
          <CRow className="g-3">
            <CCol md={6}>
              ...
              <CFormInput
                ...
                placeholder="Ej: AGENDAR_CITA"
                disabled={editingIntent !== null}
              />
            </CCol>
            ...
            <CCol md={12}>
              ...
              <CFormTextarea
                ...
                onChange={handleChange}
                placeholder="Describe cuándo se usa esta intención."
              />
            </CCol>
        ...
        <CModalFooter>
          ...
          <CButton color="primary" onClick={guardarIntencion} disabled={saving}>
            {saving ? (
              <>
